...
using WebCafeteria.Web.Models;
using WebCafeteria.Datos;
using System.Web.Helpers;

namespace WebCafeteria.Web.Helpers
{
    public class UsersHelper : IDisposable
    {
        private static readonly ApplicationDbContext UserContext = new ApplicationDbContext();
        private static readonly CafeteriaDbContext Db = new CafeteriaDbContext();
        ...
        public static async Task PasswordRecovery(string email)
        {
            ...
            var user = Db.Clientes.FirstOrDefault(tp => tp.Email == email);
            if (user == null)
            {
                return;
            }
