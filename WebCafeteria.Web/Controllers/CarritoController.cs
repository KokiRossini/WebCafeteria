...
namespace WebCafeteria.Web.Controllers
{
    public class CarritoController : Controller
    {
        ...
        private readonly IServiciosTamañosProductos _serviciosTamañosProductos;
        private readonly IServiciosClientes _serviciosClientes;
        private readonly IServiciosVentas _serviciosVentas;
        private readonly IMapper _mapper;
        public CarritoController(IServiciosTamañosProductos serviciosTamañosProductos,
            IServiciosCarrito serviciosCarrito,
            IServiciosClientes serviciosClientes,
            IServiciosVentas serviciosVentas)
        {
            _serviciosTamañosProductos = serviciosTamañosProductos;
        ...
        [HttpPost]
        public ActionResult AddToCart(int tamañoProductoId,string returnUrl, int cantidad = 1)
        {
            var tamañoProductoDto = _serviciosTamañosProductos.GetTamañoProductoPorId(tamañoProductoId);
            if (tamañoProductoDto.Stock>=cantidad)
            {
                ItemCarrito itemCarrito = new ItemCarrito
                {
                    TamañoProductoId = tamañoProductoDto.TamañoProductoId,
                    UserName = User.Identity.Name,
                    NombreProducto = tamañoProductoDto.producto.NombreProducto,
                    NombreTamaño=tamañoProductoDto.tamaño.NombreTamaño,
                    PrecioUnitario = tamañoProductoDto.PrecioUnitario,
                    Cantidad = cantidad,
                ...
                _serviciosTamañosProductos.ActualizarUnidadesEnPedido(tamañoProductoId, cantidad);
        ...
        public ActionResult CancelOrder()
        {
            ...
            {
                _serviciosCarritos.Borrar(User.Identity.Name, item.TamañoProductoId);
                _serviciosTamañosProductos.ActualizarUnidadesEnPedido(item.TamañoProductoId, -(item.Cantidad));
            }
            _carrito.Clear();
            Session["carrito"] = _carrito;
        ...
        public ActionResult RemoveFromCart(int tamañoProductoId, string returnUrl)
        {
            int cantidad = 0;
            _carrito = GetCarrito();
            foreach (var item in _carrito.GetItems())
            {
                if (item.TamañoProductoId==tamañoProductoId)
                {
                    cantidad = item.Cantidad;
                }
            }
            _carrito.RemoveFromCart(tamañoProductoId);
            Session["carrito"] = _carrito;

                _serviciosCarritos.Borrar(User.Identity.Name, tamañoProductoId);
            _serviciosTamañosProductos.ActualizarUnidadesEnPedido(tamañoProductoId, -(cantidad));
        ...
        private PaymentResult CheckOut(CheckOutVm model)
        {
            ...
            foreach (var item in model.Carrito.Items)
            {
                DetalleVenta detalleVenta = new DetalleVenta()
                {
                    ...
                    TamañoProductoId = item.TamañoProductoId,
                    Cantidad = item.Cantidad,
